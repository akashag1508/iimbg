...
df
#ggplots
ggplot(df, aes(x=wt, y= mpg)) +geom_point(aes(color = am, size = hp, shape = gear))
?mtcars
...
ggplot(df%>%group_by(cyl,gear) %>% summarise(CNT=n()),aes(x=cyl, y=CNT, fill=gear)) + geom_bar(stat='identity', position=position_dodge2(.7)) +geom_text(aes(label=CNT)) + facet_grid(carb ~ am, scale ='free')
